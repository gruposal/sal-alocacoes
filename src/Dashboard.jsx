    ...
    <div className="space-y-5">
      ...
      {hasData && activeTab === "semana" && (
        <div className="space-y-5">
          ...
          <div className={card}>
            <div className="px-4 py-3 border-b border-[var(--border-subtle)] flex flex-wrap items-center gap-3">
              <h3 className="font-semibold text-[15px] flex-1">Alocações da Semana</h3>
              {semCcs.length > 1 && (
                <select value={semCcFilter} onChange={e => setSemCcFilter(e.target.value)} className={`${selectCls} w-auto text-[13px]`}>
                  <option value="">Todos os CCs</option>
                  {semCcs.map(cc => <option key={cc} value={cc}>{cc}</option>)}
                </select>
              )}
              <div className="inline-flex bg-[var(--surface-alt)] rounded-md p-0.5 gap-0.5 border border-[var(--border-subtle)]">
                {[{k:'person',label:'Pessoa'},{k:'project',label:'Projeto'}].map(o => (
                  <button key={o.k} onClick={() => setSemSort(o.k)}
                    className={`px-3 py-1 rounded-[7px] text-[12px] font-medium transition-all ${semSort===o.k ? 'bg-[var(--surface)] text-[var(--text-1)] shadow-sm' : 'text-[var(--text-2)]'}`}>
                    {o.label}
                  </button>
                ))}
              </div>
            ...
            {!semFiltered.length ? (
              <div className="px-5 py-10 text-[14px] text-[var(--text-3)] text-center">
                Nenhum lançamento para a semana {selectedWeek}/{selectedYear}.
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full">
                    ...
                    {semFiltered.map((r, i) => {
                      ...
                      return (
                        <tr key={r._taskId || i} className="hover:bg-[var(--surface-alt)] transition-colors">
                          ...
                          <td className={`${td} text-[var(--text-2)]`}>{r.Project}</td>
                  ...
                  <tfoot>
                    <tr className="border-t-2 border-[var(--border-subtle)] bg-[var(--surface-alt)]/50">
                      <td colSpan={3} className={`${td} text-[11px] font-semibold uppercase tracking-wider text-[var(--text-3)]`}>Total</td>
                      <td className={`${td} text-right tabular-nums font-semibold`}>{semFiltered.reduce((s,r) => s+(Number(r.Hours_Forecast)||0),0)}h</td>
                      <td className={`${td} text-right tabular-nums font-semibold`}>{
                        semFiltered.some(r => r.Hours_Consolidated != null)
                          ? semFiltered.reduce((s,r) => s+(Number(r.Hours_Consolidated)||0),0) + 'h'
                          : '—'
                      }</td>
                      <td />
                    </tr>
